...
function agregarAlCarrito(producto) {
  const memoria = localStorage.getItem("juegos");
  ...
  producto = {
    ...
    cantidad: Number(producto.cantidad) || undefined,
  };
...
document.addEventListener("DOMContentLoaded", () => {
  const memoriaInicial = localStorage.getItem("juegos");
  if (memoriaInicial) {
    actualizarContadorCarrito(JSON.parse(memoriaInicial));
  }
});
...
function mostrarCarrito() {
  ...
  if (!carritoItems || !carritoVacio || !btnVaciarContainer || !subtotalEl || !totalEl) {
  ...
  carritoItems.onclick = (e) => {
    ...
    if (minusId) {
      item.cantidad--;
      if (item.cantidad <= 0) {
        carritoActual = carritoActual.filter((x) => Number(x.id_producto) !== Number(item.id_producto));
      }
    }
    ...
    if (plusId) {
      if (item.cantidad < item.stock) {
        ...
        Swal.fire({
          ...
          text: `No hay más unidades  ${item.nombre}`,
          icon: "warning",
          confirmButtonText: "Entendido",
        });
        return;
    ...
    localStorage.setItem("juegos", JSON.stringify(carritoActual));
...
if (typeof peticion === "function") peticion();